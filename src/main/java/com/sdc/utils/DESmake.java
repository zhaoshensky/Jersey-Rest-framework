...
public class DESmake
{
	...
	// 密钥是16位长度的byte[]进行Base64转换后得到的字符串
		public static String key ="Lu6/1aqfXJIeUMXa4Z6mzw==";
		...
		public static String encrypt(String xmlStr) {
			...
			byte[] md5Hasn = null;
			try {
				md5Hasn = DESmake.MD5Hash(encrypt, 0, encrypt.length);
			} catch (Exception e) {
				e.printStackTrace();
			}
			// 组合消息体
			byte[] totalByte = DESmake.addMD5(md5Hasn, encrypt);

			// 取密钥和偏转向量
			byte[] key = new byte[8];
			byte[] iv = new byte[8];
			getKeyIV(DESmake.key, key, iv);
			...
			byte[] temp = null;
			try {
				temp = DESmake.DES_CBC_Encrypt(totalByte, deskey, ivParam);
			} catch (Exception e) {
				e.printStackTrace();
			}
		...
		public static String decrypt(String xmlStr) throws Exception {
			...
			byte[] temp = null;
			try {
				temp = DESmake.DES_CBC_Decrypt(encBuf, deskey, ivParam);
			} catch (Exception e) {
				e.printStackTrace();
			}
			...
			byte[] md5Hash = null;
			try {
				md5Hash = DESmake.MD5Hash(temp,16,temp.length-16);
			} catch (Exception e) {
				e.printStackTrace();
			}
			// 进行解密校检
