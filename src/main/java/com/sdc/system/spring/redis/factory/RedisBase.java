package com.sdc.system.spring.redis.factory;
...
import com.sdc.system.spring.redis.factory.pool.RedisPool;
...
public class RedisBase implements RedisFactory{
	...
	*
	* @param redisType 某字符串
	*        Data类型：通用数据redis实例  此部分仅限于基础数据类的使用  
			 Market类型:营销类redis实例 例如：积分、优惠券 
		     Cache类型：缓存类redis实例  例如：需要缓存数据
		     FeiheCrm类型：缓存类飞鹤 手机号:会员id示例 
	*/
    public RedisBase(String redisType){
    	
    	
    	String className = RedisPool.class.getPackage().getName() + ".Redis" + redisType + "Pool";
